...
.defines(function(){
	...
	RoboTomGame = ig.Game.extend({
		...
		accessCount: 0,
		gameOver: false,
		startingLevel: CURRENT_LEVEL_INDEX,
		...
		_updateAccessCount: function() {
			if (typeof this.portal != 'undefined' && this.levelIndex != this.portal.levelIndex) {
		...
		_updateLevelInfo: function() {
			...
			if (typeof this.portal === 'undefined') {
				...
				if (this.levelName != this.portal.levelName) {
					...
					this.accessCount = (this.getEntitiesByType( EntityAccesskey )).length;
					CURRENT_LEVEL_INDEX = this.levelIndex;
					NEXT_LEVEL_INDEX = this.levelNext;
					ON_CLEAR = false;
					console.log('current level index', CURRENT_LEVEL_INDEX);
					console.log('next level index', NEXT_LEVEL_INDEX);
				}	
			}
